...
\log(1 + \mu(\gamma))$; and the hierarchy of hypotheses
...
the Fernández--Procacci form. The development is about 730 lines of Lean,
compiles against current \textsf{mathlib}, and contains no \texttt{sorry}
placeholders. Section~\ref{sec:kp} documents a claim made in an earlier
version of this note that the formalisation subsequently refuted.
...
The Fernández--Procacci \emph{theorem} — nonvanishing of $Z$ under the
weakest of the three hypotheses \cite{FP07} — is deliberately not claimed;
its refined induction is future work, as is the convergent Ursell-series
expansion of $\log Z$ itself, which requires tree-graph bounds beyond the
volume-linear control of Theorem~\ref{thm:log}.
