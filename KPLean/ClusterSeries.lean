...
die Baumschranken in Reihenschranken übersetzt. -/
theorem norm_intCast_eq_abs (m : ℤ) : ‖(m : K)‖ = |(m : ℝ)| := by
  rw [← RCLike.ofReal_intCast, RCLike.norm_ofReal]
...
Gewichtsprodukt. -/
noncomputable def clusterOrderSum (w : ι → K) (Λ : Finset ι) (n : ℕ) : K :=
...
noncomputable def clusterCoeff (w : ι → K) (Λ : Finset ι) (n : ℕ) : K :=
  clusterOrderSum P w Λ n / (Nat.factorial (n + 1) : K)
  ...
  calc ‖clusterOrderSum P w Λ n‖ / (Nat.factorial (n + 1) : ℝ)
    ...
    _ ≤ Real.exp ((n : ℝ) + 1) * (∑ x ∈ Λ, ‖w x‖) ^ (n + 1) :=
        mul_le_mul_of_nonneg_right (pow_div_factorial_le n)
          (pow_nonneg (Finset.sum_nonneg fun x _ => norm_nonneg _) _)
...
theorem summable_abs_clusterCoeff (w : ι → K) (Λ : Finset ι)
    (h : Real.exp 1 * ∑ x ∈ Λ, ‖w x‖ < 1) :
    Summable fun n => ‖clusterCoeff P w Λ n‖ := by
  have hr0 : (0 : ℝ) ≤ Real.exp 1 * ∑ x ∈ Λ, ‖w x‖ :=
    mul_nonneg (Real.exp_pos 1).le (Finset.sum_nonneg fun x _ => norm_nonneg _)
...
`|clusterSeries| ≤ r / (1 - r)`. -/
theorem abs_clusterSeries_le (w : ι → K) (Λ : Finset ι)
    (h : Real.exp 1 * ∑ x ∈ Λ, ‖w x‖ < 1) :
    ‖clusterSeries P w Λ‖
      ≤ (Real.exp 1 * ∑ x ∈ Λ, ‖w x‖)
...
noncomputable def pinnedOrderSum (w : ι → K) (Λ : Finset ι) (γ₀ : ι)
    (n : ℕ) : ℝ :=
  ∑ γ ∈ (Fintype.piFinset fun _ : Fin (n + 1) => Λ).filter
      fun γ => γ 0 = γ₀,
    ‖(ursellInt P γ : K)‖ * ∏ i, ‖w (γ i)‖

theorem pinnedOrderSum_nonneg (w : ι → K) (Λ : Finset ι) (γ₀ : ι) (n : ℕ) :
    0 ≤ pinnedOrderSum P w Λ γ₀ n :=
  Finset.sum_nonneg fun _γ _ => mul_nonneg (norm_nonneg _)
    (Finset.prod_nonneg fun _i _ => norm_nonneg _)
...
theorem pinnedOrderSum_le (w : ι → K) (Λ : Finset ι) (γ₀ : ι) (n : ℕ) :
  ...
  have hterm : ∀ γ ∈ (Fintype.piFinset fun _ : Fin (n + 1) => Λ).filter
    ...
    refine mul_le_mul_of_nonneg_right (mul_le_mul_of_nonneg_right ?_
      (norm_nonneg _)) (Finset.prod_nonneg fun i _ => norm_nonneg _)
    calc ‖(ursellInt P γ : K)‖ = ((|ursellInt P γ| : ℤ) : ℝ) := by
          rw [norm_intCast_eq_abs, Int.cast_abs]
      _ ≤ ((((n + 1) ^ n : ℕ) : ℤ) : ℝ) := by
          exact_mod_cast abs_ursellInt_le_pow P γ
  ...
  have hsum : ∑ γ ∈ (Fintype.piFinset fun _ : Fin (n + 1) => Λ).filter
    ...
    · have hempty : (Fintype.piFinset fun _ : Fin (n + 1) => Λ).filter
      ...
      exact pow_nonneg (Finset.sum_nonneg fun x _ => norm_nonneg _) n
...
`1/n!`-Normierung. -/
theorem pinnedCoeff_le (w : ι → K) (Λ : Finset ι) (γ₀ : ι) (n : ℕ) :
    ...
    _ ≤ Real.exp ((n : ℝ) + 1) * (‖w γ₀‖ * (∑ x ∈ Λ, ‖w x‖) ^ n) :=
        mul_le_mul_of_nonneg_right (pow_div_factorial_le' n)
          (mul_nonneg (norm_nonneg _)
            (pow_nonneg (Finset.sum_nonneg fun x _ => norm_nonneg _) n))
...
theorem tsum_pinned_le (w : ι → K) (Λ : Finset ι) (γ₀ : ι)
    (h : Real.exp 1 * ∑ x ∈ Λ, ‖w x‖ < 1) :
